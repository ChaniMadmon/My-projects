...
export class KineretComponent implements OnInit, OnDestroy {
  ...
  createChartFromRange(data: any[]) {
    ...
    const pixelStep = 4.5;                               // כמה פיקסלים כל פריים
    ...
    const animate = () => {
      this.surfPos += this.surfStep * surfDir;

      // אם הגיע לקצה – הפוך כיוון
      if (this.surfPos >= labels.length - 1) {
        this.surfPos = labels.length - 1;
        surfDir = -1;
      }
      if (this.surfPos <= 0) {
        this.surfPos = 0;
